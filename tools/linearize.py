...
class LegPoseSolver:
    ...
    def leg_com(self, data):
        ...
        for bid in self._bids:
            ...
            tot+=m; com+=m*(data.xpos[bid]+data.xipos[bid])
        com/=tot; d=com-hip
...
def linearize_at_length(L_l, L_r, r_com, c_x, M_func, Bq_func, V_func):
    ...
    def dV(q_a):
        ...
        return grad
    try:
        sol=fsolve(dV,[0,0,0],maxfev=200,xtol=1e-12)
        th_eq,thl_eq,thr_eq=float(sol[0]),float(sol[1]),float(sol[2])
    except: th_eq=thl_eq=thr_eq=0.0
...
def poly2d_terms(p, L_l, L_r):
    ...
    Returns list: [1, L_l, L_r, L_l², L_l·L_r, L_r², ..., L_l^p, ..., L_r^p]
    """
    out = []
    for d in range(p + 1):
        for i in range(d + 1):
...
def poly2d_design_matrix(p, L_l_vec, L_r_vec):
    """Build design matrix where each row is poly2d_terms(p, L_l[k], L_r[k])."""
    N = len(L_l_vec)
    n_terms = (p + 1) * (p + 2) // 2
...
def fit_poly_2d(p, L_l_vec, L_r_vec, K_2d):
    """Fit p-th order 2D polynomial to each K[i,j] over (L_l,L_r) grid.
...
def eval_poly_2d(p, coeff, L_l, L_r):
    """Evaluate 2D polynomial at (L_l, L_r). coeff shape: (4, 10, n_terms)."""
...
def export_c_header_2d(p, coeff, filepath):
    """Export 6th-order 2D polynomial gains as C header for MCU.

    Format: flat float32 array, runtime eval function included.
    """
    no, ns, n_terms = coeff.shape  # (4, 10, 28)
    flat = coeff.ravel()  # row-major: coeff[i,j,k] → index = i*ns*n_terms + j*n_terms + k
    n_floats = len(flat)
    n_kb = n_floats * 4 / 1024

    lines = []
    lines.append("// Auto-generated 6th-order 2D LQR gain table for asymmetric leg lengths.")
    ...
    lines.append("    // Precompute powers Ll[i]=L_left^i, Lr[i]=L_right^i  (i=0..6)")
    lines.append("    float Ll[7], Lr[7];")
    lines.append("    Ll[0] = 1.0f; Lr[0] = 1.0f;")
    lines.append("    Ll[1] = L_left; Lr[1] = L_right;")
    lines.append("    for (int i = 2; i <= 6; i++) {")
    lines.append("        Ll[i] = Ll[i-1] * L_left;")
    lines.append("        Lr[i] = Lr[i-1] * L_right;")
    lines.append("    }")
    lines.append(f"    // Monomial terms: Ll[d-a] * Lr[a] for d=0..{p}")
    ...
    for d in range(p + 1):
        for a in range(d + 1):
            b = d - a
            if b == 0 and a == 0:
                lines.append(f"    terms[{idx}] = 1.0f;")
            elif b == 0:
                lines.append(f"    terms[{idx}] = Lr[{a}];")
            elif a == 0:
                lines.append(f"    terms[{idx}] = Ll[{b}];")
            else:
                lines.append(f"    terms[{idx}] = Ll[{b}] * Lr[{a}];")
            idx += 1
    lines.append("")
